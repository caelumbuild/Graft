  ...
  try {
    const mine = readdirSync(cacheDir)
      ...
      .map((f) => {
        ...
        return { full, mtimeMs: statSync(full).mtimeMs, protected: full === protectedPath };
      })
      .sort((a, b) => Number(b.protected) - Number(a.protected) || b.mtimeMs - a.mtimeMs || a.full.localeCompare(b.full));
