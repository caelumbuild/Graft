...
test('cursor/gemini/kiro get repo-local JSON entries', () => {
  ...
  assert.deepEqual(cursor.mcpServers.graft, { command: 'caelum-graph', args: ['mcp'] });
...
test('agents id: codex TOML + opencode JSON, gated on home dirs', () => {
  ...
  assert.match(toml, /command = "caelum-graph"/);
...
test('serverEntry always uses the installed Caelum binary', () => {
  assert.deepEqual(serverEntry({ onPath: true }), { command: 'caelum-graph', args: ['mcp'] });
  assert.deepEqual(serverEntry({ onPath: false }), { command: 'caelum-graph', args: ['mcp'] });
  assert.ok(!serverEntry().command.startsWith('/'), 'shared config never embeds a machine path');
});
